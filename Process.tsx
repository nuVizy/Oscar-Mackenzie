  ...
  return (
    <section id="process" className="py-20 md:py-32 px-6 md:px-12 max-w-[1600px] mx-auto border-t border-white/5">
      <div className="mb-12 md:mb-20">
         <span className="text-blue-500 font-mono text-[10px] font-black tracking-widest uppercase block mb-4">How We Work</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-white/5 border border-white/5">
        {steps.map((step) => (
          <div key={step.num} className="bg-black p-8 md:p-10 space-y-8 group hover:bg-white/[0.02] transition-colors relative overflow-hidden">
            <div className="absolute -right-4 -bottom-4 text-7xl md:text-[8rem] font-black text-white/[0.02] group-hover:text-blue-500/[0.05] transition-colors leading-none select-none">
              {step.num}
            </div>
            <div className="space-y-4 relative z-10">
              <span className="text-blue-500 font-mono text-[10px] font-black tracking-widest">STEP {step.num}</span>
            ...
            <div className="w-8 h-[1px] bg-blue-500/30 group-hover:w-full transition-all duration-500"></div>
          </div>
        ))}
      </div>
    </section>
